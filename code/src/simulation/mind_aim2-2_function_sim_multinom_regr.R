...
generate.data <- function(params, # List of parameters 
  ...
  ##############################################################################
  # Generate lognormal distribution for antibiotic use if parameters are provided
  if(!is.null(abx_params)){
    abx_distr <- list()
    for(i in 1:length(abx_params)){
      abx_distr[[i]] <- rlnorm(n_facilities*n_obs_per_facility, mean = abx_params[[i]][, "mean"], sd = abx_params[[i]][, "sd"])
    }
  }
  names(abx_distr) <- names(abx_params)
  
  if(length(abx_params) != 0 & is.null(abx_distr)){
  ...
  lp <- list()
  constant <- beta[1]*year + beta[2]*day + beta[3]*month + facility_effect + sum(sigma * c(census_region, facility_rurality, complexitylevel))
  temp <- NULL
  for(i in 1:length(antibiograms)){
    lp[[i]] <- 0
    temp[[i]] <- beta0[i]
    for(j in 1:length(abx_distr)){
      temp[[i]] <- temp[[i]] + gamma[j] * abx_distr[[j]] * ab_outcomes[[i]][j]
    }
  ...
  ##############################################################################
  # Create simulation dataset as data frame 
